...
--Add new column helpfull in Oracle - SQL Server migration
ALTER TABLE PRODUCTS ADD IsCopied number(1) DEFAULT 0 NOT NULL;
ALTER TABLE PRODUCTS ADD IsDeleted number(1) DEFAULT 0 NOT NULL;

ALTER TABLE MEASURES ADD IsCopied number(1) DEFAULT 0 NOT NULL;
ALTER TABLE MEASURES ADD IsDeleted number(1) DEFAULT 0 NOT NULL;

ALTER TABLE PRODUCTSTYPES ADD IsCopied number(1) DEFAULT 0 NOT NULL;
ALTER TABLE PRODUCTSTYPES ADD IsDeleted number(1) DEFAULT 0 NOT NULL;

ALTER TABLE VENDORS ADD IsCopied number(1) DEFAULT 0 NOT NULL;
ALTER TABLE VENDORS ADD IsDeleted number(1) DEFAULT 0 NOT NULL;
...
--Create stored procedures for returning only records which is not deleted and copied
/* Create package */
...
--Return records from VENDORS
 CREATE OR REPLACE PROCEDURE VENDOR_RTN_RECORDSET
 (
 ...
 SELECT 
  v.VENDORNAME
 FROM VENDORS v
 WHERE v.ISCOPIED < 1 AND v.ISDELETED < 1;

END VENDOR_RTN_RECORDSET;
/

--Return records from MEASURES
 CREATE OR REPLACE PROCEDURE MEASURES_RTN_RECORDSET
 (
 p_ResultSet        OUT  TYPES.cursorType
 )
 AS
 ...
 SELECT 
  m.MEASURENAME
 FROM MEASURES m
 WHERE m.ISCOPIED < 1 AND m.ISDELETED < 1;

END MEASURES_RTN_RECORDSET;
/


--Return records from PRODUCTSTYPES
 CREATE OR REPLACE PROCEDURE PRODUCTSTYPES_RTN_RECORDSET
 (
 p_ResultSet        OUT  TYPES.cursorType
 )
 AS
 ...
 WHERE pt.ISCOPIED < 1 AND pt.ISDELETED < 1;

END PRODUCTSTYPES_RTN_RECORDSET;
/
...
--Return records from PRODUCTS
CREATE OR REPLACE PROCEDURE PRODUCTS_RTN_RECORDSET
 (
 p_ResultSet        OUT  TYPES.cursorType
 )
 AS
 ...
 OPEN p_ResultSet FOR
    ...
    WHERE p.ISCOPIED < 1 AND p.ISDELETED < 1;  
END PRODUCTS_RTN_RECORDSET;
/
...
-- Testing-----
 ...
 EXEC VENDOR_RTN_RECORDSET( :resultSet);
 PRINT resultSet;
 
 VARIABLE resultSet  REFCURSOR
 EXEC MEASURES_RTN_RECORDSET( :resultSet);
 PRINT resultSet;
 
 VARIABLE resultSet  REFCURSOR
 EXEC PRODUCTSTYPES_RTN_RECORDSET( :resultSet);
 PRINT resultSet;
 
 VARIABLE resultSet  REFCURSOR
 EXEC PRODUCTS_RTN_RECORDSET( :resultSet);
 PRINT resultSet;
