namespace SupermarketChain.ConsoleClient
{
    ...
    public static class OracleDBReplication
    {
        ...
        ///</summary>
        public static void UpdateProductsFromOracle(OracleEntities oracleContext, SupermarketContext msSqLcontext)
        {
            ...
            if (products.Count > 0)
            {
                ...
                foreach (var product in products)
                {
                    ...
                    var vendorId = msSqLcontext.Vendors
                        .Where(v => v.VendorName == product.VENDORNAME)
                        .Select(v => v.Id).FirstOrDefault();

                    var measureId = msSqLcontext.Measures
                        .Where(m => m.MeasureName == product.MEASURENAME)
                        .Select(m => m.Id).FirstOrDefault();

                    var typeId = msSqLcontext.ProductTypes
                        .Where(pt => pt.TypeName == product.TYPENAME)
                        .Select(pt => pt.Id).FirstOrDefault();
        ...
        ///Second it inserts record into MS SQL Method and mark them as copied into Oracle model
        ///</summary>
        public static void UpdateProductsTypesFromOracle(OracleEntities oracleContext, SupermarketContext msSQLcontext)
        {
        ...
        ///Second it inserts record into MS SQL Method and mark them as copied into Oracle model
        ///</summary>
        public static void UpdateMeasuresFromOracle(OracleEntities oracleContext, SupermarketContext msSQLcontext)
        {
        ...
        ///Second it inserts record into MS SQL Method and mark them as copied into Oracle model
        ///</summary>
        public static void UpdateVendorsFromOracle(OracleEntities oracleContext, SupermarketContext msSQLcontext)
        {
